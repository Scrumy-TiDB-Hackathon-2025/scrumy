...
import logging
from .retry_manager import retry_manager, RetryableError
    ...
    async def create_tasks_from_ai_results(self, ai_tasks: List[Dict],
        ...
        for i, integration_task in enumerate(integration_tasks):
            try:
                ...
                # Create task using tools registry with retry mechanism
                result = await retry_manager.execute_with_retry(
                    self.tools_registry.create_task_everywhere,
                    "task_creation",
                ...
                if result.get("success") and task_result.get("task_created"):
                    ...
                    if result.get('retries_exhausted'):
                        ...
                        error_msg = f"Task {i+1} creation failed: {error_details}"
